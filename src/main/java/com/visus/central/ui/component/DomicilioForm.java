...
public class DomicilioForm extends VerticalLayout {
	...
	public DomicilioForm() {
		...
		FormLayout layout = new FormLayout(calleNumero, // Fila 1: Calle + Número
		...
		layout.setResponsiveSteps(new FormLayout.ResponsiveStep("0", 1), new FormLayout.ResponsiveStep("600px", 2),
	...
	// Método para configurar las localidades disponibles (not null
	public void setLocalidades(java.util.List<Localidad> localidades) {
		...
		List<Localidad> localidadesFiltradas = this.localidadesDisponibles.stream().filter(l -> l != null)
				.collect(Collectors.toList());

		localidad.setItems(localidadesFiltradas);
		localidad.setItemLabelGenerator(loc -> {
			if (loc == null)
				return "";
			return loc.getNombre() != null ? loc.getNombre() : "";
		});
	}
	...
	public void setDomicilio(Domicilio domicilio) {
		...
		calle.setValue(domicilio.getCalle() != null ? domicilio.getCalle() : "");
		numero.setValue(domicilio.getNumero() != null ? domicilio.getNumero().toString() : "");
		barrio.setValue(domicilio.getBarrio() != null ? domicilio.getBarrio() : "");
		manzana.setValue(domicilio.getManzana() != null ? domicilio.getManzana() : "");
		casa.setValue(domicilio.getCasa() != null ? domicilio.getCasa() : "");
		sector.setValue(domicilio.getSector() != null ? domicilio.getSector() : "");
		oficina.setValue(domicilio.getOficina() != null ? domicilio.getOficina() : "");
		lote.setValue(domicilio.getLote() != null ? domicilio.getLote() : "");
		tipoDomicilio.setValue(domicilio.getTipoDomicilio());

		// Cargar localidad si existe
		if (domicilio.getIdLocalidad() != null && localidadesDisponibles != null) {
			Localidad localidadSeleccionada = localidadesDisponibles.stream()
					.filter(l -> l != null && l.getIdlocalidad() != null)
					.filter(l -> l.getIdlocalidad().equals(domicilio.getIdLocalidad())).findFirst().orElse(null);
