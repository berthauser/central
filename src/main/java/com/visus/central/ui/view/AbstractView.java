...
public abstract class AbstractView<T> extends VerticalLayout {
	...
	// NUEVO: Método que las subclases pueden sobreescribir para agregar botones de
	// acción
	protected void addActionButtons(Grid<T> grid) {
	...
	private void actualizarGrillaOptimista(T modelGuardado) {
		...
		boolean existe = itemsList.stream().anyMatch(item -> getId(item).equals(idGuardado));

		if (existe) {
			// Modificación: reemplazar el item existente
			itemsList.replaceAll(item -> getId(item).equals(idGuardado) ? modelGuardado : item);
		} else {
