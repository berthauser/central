...
public class ArticulosView extends AbstractView<Articulo> {
	...
	// MODIFICADO: Constructor - deferir carga de datos
	...
	// MODIFICADO: También sobrescribir guardar y eliminar para usar paginación
	@Override
    protected void guardar(Articulo model) {
        ...
        try {
            ...
            // Recargar los datos paginados después de guardar
            cargarDatosPaginados();
	...
	protected List<Grid.Column<Articulo>> buildColumns(Grid<Articulo> grid) {
		return List.of(
				grid.addColumn(Articulo::getCodigo_interno)
				.setHeader("CÓDIGO INTERNO")
				.setAutoWidth(true)
				.setResizable(true),

				grid.addColumn(Articulo::getCodigo_barra)
				.setHeader("CÓDIGO DE BARRA")
				.setAutoWidth(true)
				.setResizable(true),

				grid.addColumn(Articulo::getDescripcion)
				.setHeader("DESCRIPCIÓN")
				.setAutoWidth(true)
				.setResizable(true),

				// STOCK formateado con separadores de miles
				grid.addColumn(articulo -> FormatoUtils.formatNumero(articulo.getStock()))
				.setHeader("STOCK")
				.setAutoWidth(true)
				.setResizable(true)
				.setTextAlign(ColumnTextAlign.END), // Alinear a la derecha
				...
				.setHeader("STOCK MÁXIMO")
				.setAutoWidth(true)
				.setResizable(true)
				.setTextAlign(ColumnTextAlign.END), // Alinear a la derecha

				grid.addColumn(articulo -> articulo.getLinea() != null ? 
						articulo.getLinea().getDescripcion() : "")
				.setHeader("LÍNEA")
				.setAutoWidth(true)
				.setResizable(true),
				...
				grid.addColumn(articulo -> {
					if (articulo.getLinea() != null && 
							articulo.getLinea().getRubro() != null) {
				...
				.setAutoWidth(true)
				.setResizable(true)
				.setFlexGrow(1),  // Puede crecer un poco más

				grid.addColumn(articulo -> articulo.getMedida() != null ? 
						articulo.getMedida().getDescripcion() : "")
				.setHeader("MEDIDA")
				.setAutoWidth(true)
				.setResizable(true),

				grid.addColumn(articulo -> articulo.getPresentacion() != null ? 
						articulo.getPresentacion().getDescripcion() : "")
				.setHeader("PRESENTACIÓN")
				.setAutoWidth(true)
				.setResizable(true),

				// Columna 1: Descripción de la Alícuota (mostrar exactamente lo que hay en BD)
				grid.addColumn(articulo -> articulo.getAlicuota() != null ? 
						articulo.getAlicuota().getDescripcion() : "Sin Asignar")
				.setHeader("ALÍCUOTA")
				.setAutoWidth(true)
				.setResizable(true),
				...
				.setAutoWidth(true)
				.setResizable(true)
				.setTextAlign(ColumnTextAlign.END), // Agregar esta línea
				...
				grid.addColumn(articulo -> {
					...
					BigDecimal gravamen = articulo.getAlicuota() != null ? 
							articulo.getAlicuota().getGravamen() : BigDecimal.ZERO;
				...
				grid.addColumn(articulo -> {
					...
					if (margen != null) {
						BigDecimal factorMargen = BigDecimal.ONE.add(
								margen.divide(new BigDecimal("100"))
								);
					...
					if (gravamen != null && gravamen.compareTo(BigDecimal.ZERO) > 0) {
						BigDecimal factorGravamen = BigDecimal.ONE.add(
								gravamen.divide(new BigDecimal("100"))
								);
				...
				grid.addComponentColumn(articulo -> {
					...
					if (articulo.getEstado() == JpaArticuloEntity.Estado.NoDisponible) {
						Button reactivar = new Button("Reactivar", new Icon(VaadinIcon.CHECK));
	...
	private void mostrarConfirmacionEliminacion(Articulo articulo) {
		...
		Span mensaje = new Span("¿Está seguro de dar de baja el Artículo \"" + 
				articulo.getDescripcion() + "\"? El artículo cambiará a estado 'No Disponible'.");
...
//				actualizarLista();
	...
	private void setupPagination() {
	    ...
	    // El listener puede usar cargarDatosPaginados porque pageSizeCombo ya está inicializado
		...
		// Simplemente crea el tooltip
		Tooltip.forComponent(pageSizeCombo)
		.withText("Valor por defecto: 50 - Registros por página")
		.withPosition(Tooltip.TooltipPosition.TOP);
		...
		HorizontalLayout paginationLayout = new HorizontalLayout(
				firstPageButton, prevPageButton, pageInfo, 
	...
	private void cargarDatosPaginados() {
	    try {
	        ...
	        Pageable pageable = PageRequest.of(
	                currentPage, 
	                pageSizeValue, // Usar variable en lugar de llamada directa
	...
	private void updatePaginationControls() {
	    ...
	    pageInfo.setText(String.format("Página %d de %d - Mostrando %d-%d de %d registros", 
	            currentPage + 1, 
	    ...
	    if (firstPageButton != null) firstPageButton.setEnabled(currentPage > 0);
	    if (prevPageButton != null) prevPageButton.setEnabled(currentPage > 0);
	    if (nextPageButton != null) nextPageButton.setEnabled(currentPage < totalPages - 1);
	    if (lastPageButton != null) lastPageButton.setEnabled(currentPage < totalPages - 1);
	}
...
//				actualizarLista();
	...
	private void buscarPorCodigoBarra(String codigoBarra) {
	    ...
	    try {
	        ...
	        if (articuloOpt.isPresent()) {
	            ...
	            Notification.show("✓ Artículo encontrado: " + articulo.getDescripcion(), 
	            ...
	            Notification.show("✗ No se encontró el código: " + codigoBarra, 
	                3000, Notification.Position.BOTTOM_CENTER)
	...
	private void setPaginationDisabled(boolean disabled) {
	    if (firstPageButton != null) firstPageButton.setEnabled(!disabled);
	    if (prevPageButton != null) prevPageButton.setEnabled(!disabled);
	    if (nextPageButton != null) nextPageButton.setEnabled(!disabled);
	    if (lastPageButton != null) lastPageButton.setEnabled(!disabled);
	    if (pageSizeCombo != null) pageSizeCombo.setEnabled(!disabled);
	}
