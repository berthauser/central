...
public class ComprobanteView extends AbstractView<Comprobante> {
	...
	protected List<Grid.Column<Comprobante>> buildColumns(Grid<Comprobante> grid) {
		return List.of(grid.addColumn(Comprobante::getNombreLargo).setHeader("NOMBRE LARGO"),
				...
				grid.addColumn(Comprobante::getNumeroInicial).setHeader("NÚMERO FINAL"),
				grid.addColumn(Comprobante::getNumeroActual).setHeader("N° ACTUAL"), // NUEVO
				grid.addColumn(c -> c.getActivo() != null && c.getActivo() ? "Sí" : "No").setHeader("ACTIVO"),
				grid.addColumn(c -> c.getColumna() != null ? c.getColumna().getLabel() : "").setHeader("COLUMNA"));
	}
	...
	private void mostrarConfirmacionEliminacion(Comprobante comprobante) {
		...
		Span mensaje = new Span(
				"¿Está seguro de que desea eliminar el Comprobante \"" + comprobante.getNombreLargo() + "\"?");
